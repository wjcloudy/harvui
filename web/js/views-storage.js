...
function drawArch(keep) {
  ...
  svg.innerHTML = (STATE.data.alinks || []).map(([a, b, col]) => {
    ...
    const cx = (A.r + B.l) / 2;
    const d = `M ${A.r} ${A.y} C ${cx} ${A.y} ${cx} ${B.y} ${B.l} ${B.y}`;
...
async function viewStorage() {
  ...
  paint(`<div class="phead"><div><h2>Volumes</h2>
      <p>${v.length} Longhorn volume${v.length === 1 ? "" : "s"} · replicated block storage</p></div></div>
  ${st ? `<div class="grid g4" style="margin-bottom:18px">
    ...
    <div class="card flat"><div class="ctitle">Replica health</div>
      <div class="row" style="margin-top:10px;gap:8px;flex-wrap:wrap">
        ...
        ${st.faulted ? `<span class="tag bad">${st.faulted} faulted</span>` : ""}</div>
  ...
  <div class="card flat pad0"><div class="tblwrap"><table class="tbl"><thead><tr>
   <th>Volume</th><th>Attached to</th><th>Node</th><th>Health</th><th>Replicas</th><th>Size</th><th>Used</th><th>Last used</th>
     ...
     <td class="mono"><b>${x.replicas}</b></td><td class="mono">${x.size_gb} GB</td>
     <td class="mono dim">${x.actual_gb} GB</td>
     <td class="small dim">${x.state === "attached" ? '<span class="tag ok">in use</span>' : esc(fmtAgo(x.last_used_secs))}</td>
     </tr>`).join("") || `<tr><td colspan=8 class="empty">none</td></tr>`}
   </tbody></table></div></div>`);
}
...
async function viewEvents() {
  ...
  paint(`<div class="phead"><div><h2>Events</h2><p>Most recent cluster activity</p></div></div>
  <div class="card flat pad0"><div class="tblwrap"><table class="tbl"><thead><tr>
  ...
  ${e.map(x => `<tr><td><b>${esc(x.obj)}</b><div class="dim xs">${esc(x.ns)} · ${esc(x.kind)}</div></td>
    ...
    <td class="dim xs mono">${esc((x.time || "").replace("T", " ").replace("Z", ""))}</td></tr>`).join("")
